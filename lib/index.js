...
/**
 * Bridge failure surfaced to the host's image admission when the plugin
 * cannot take the images. `code` is a stable wire code the host maps into
 * the RPC response, so the web client shows an actionable message instead of
 * the generic "model does not support images" refusal.
 */
class EasyVisionBridgeError extends Error {
	constructor(code, message, options) {
		super(message, options);
		this.name = "EasyVisionBridgeError";
		this.code = code;
	}
}

/**
 * Resolve the live EasyVision vision model and verify it declares image
 * input, so the bridge refuses a misconfiguration with a user-facing code
 * before touching any image bytes.
 */
async function resolveVisionModel(ctx, resolved, signal) {
	let info;
	try {
		info = await ctx.llm.resolveModelInfo(resolved.provider, resolved.model, signal);
	} catch (error) {
		throw new EasyVisionBridgeError("easyvision-not-configured", `no vision model "${resolved.model}" on provider "${resolved.provider}" in your dsh model list; open Settings → EasyVision and pick one from the list`, { cause: error });
	}
	const modalities = info.inputModalities;
	if (modalities !== void 0 && !modalities.includes("image")) {
		throw new EasyVisionBridgeError("easyvision-model-text-only", `the model "${resolved.model}" on provider "${resolved.provider}" does not accept images; open Settings → EasyVision and pick a vision-capable model`);
	}
	return info;
}

/**
 * The image-admission bridge: the patched host calls this through the
 * `easyvision` service when the session's model is text-only and the user
 * attached images. It verifies the plugin is configured and resolves a
 * vision-capable model — the actual describing happens at request time via
 * {@link installRequestTransform}, so the committed message keeps its image
 * blocks and the web chat shows the picture, not a text dump. Any
 * configuration failure throws {@link EasyVisionBridgeError} with a wire
 * code the host surfaces to the client.
 *
 * @param ctx - the harness context (llm service).
 * @param config - the plugin entry config (base layer).
 * @param settingsSource - live settings-resolved source (entry or section).
 * @param content - the validated wire prompt parts (text + image).
 */
async function checkPromptContent(ctx, config, settingsSource, content) {
	const resolved = effectiveConfig(config, settingsSource);
	await resolveVisionModel(ctx, resolved);
}

/**
 * Describe one user message's image blocks through the vision model and
 * return replacement content: the original text blocks plus one text block
 * carrying the description (the message's own text is the vision question).
 * Used by the request transform, so the description reaches the text-only
 * conversation model while the committed message keeps its real images.
 *
 * @param message - a session user message whose content may hold image blocks.
 * @returns the replacement content blocks.
 */
async function describeMessageImages(ctx, config, settingsSource, message, signal) {
	const resolved = effectiveConfig(config, settingsSource);
	await resolveVisionModel(ctx, resolved, signal);
	const blocks = message.content;
	const textBlocks = blocks.filter((block) => block.type === "text" && typeof block.text === "string");
	const imageBlocks = blocks.filter((block) => block.type === "image" && block.attachment !== void 0);
	if (imageBlocks.length === 0) return blocks;
	const prompt = textBlocks.map((block) => block.text).join("\n").trim() || resolved.defaultPrompt;
	const visionMessage = createUserMessage({
		content: [
			...imageBlocks.map((block) => ({ type: "image", attachment: block.attachment })),
			{ type: "text", text: prompt }
		],
		source: { kind: "plugin", plugin: "easyvision" }
	});
	const { description, truncated } = await describeWithVisionModel(ctx, resolved, visionMessage, signal);
	const label = imageBlocks.length === 1 ? "Attached image" : `Attached images (${imageBlocks.length})`;
	const marker = `[${label} — described by ${resolved.provider}/${resolved.model} via EasyVision: ${description}]`;
	return [
		...textBlocks,
		{
			type: "text",
			text: truncated === true ? `${marker}\n\n(Note: the vision model hit its token limit; the description above may be cut short.)` : marker
		}
	];
}

/** Stable cache key for one image-bearing user message. */
function imageMessageKey(message) {
	const images = message.content.filter((block) => block.type === "image" && block.attachment !== void 0);
	const text = message.content.filter((block) => block.type === "text" && typeof block.text === "string").map((block) => block.text).join("\n");
	return `${images.map((block) => block.attachment.attachmentId).join(",")}::${text}`;
}

/**
 * Request-time transform: replace image blocks in user messages with the
 * EasyVision description when the REQUEST's model is text-only, so the
 * adapter never sees image blocks it cannot send. Vision-capable models and
 * image-free requests pass through untouched (same object). A failed vision
 * call degrades to a short note instead of failing the turn.
 *
 * @returns the request to stream — the same object when nothing changed.
 */
async function transformRequest(ctx, config, settingsSource, request, cache) {
	const messages = request.messages;
	if (!messages.some((message) => message.role === "user" && contentHasImage(message.content))) return request;
	const info = await ctx.llm.resolveModelInfo(request.provider, request.model, request.signal);
	if (info.inputModalities !== void 0 && info.inputModalities.includes("image")) return request;
	// Our own vision calls carry the easyvision plugin source; never re-describe
	// them (a text-only vision-model pick fails in the adapter instead).
	if (messages.some((message) => message.source?.kind === "plugin" && message.source.plugin === "easyvision")) return request;
	const next = [];
	let changed = false;
	for (const message of messages) {
		...
		const key = imageMessageKey(message);
		let blocks = cache.get(key);
		if (blocks === void 0) {
			try {
				blocks = await describeMessageImages(ctx, config, settingsSource, message, request.signal);
			} catch (error) {
				const cause = error instanceof Error ? error.message.replace(/^easyvision: /, "") : String(error);
				blocks = [{ type: "text", text: `[Image attached but EasyVision could not describe it: ${cause}]` }];
			}
	...
	return changed ? { ...request, messages: next } : request;
}

/**
 * Install the request-time image transform on the shared LLM runtime. The
 * agent loop dispatches through `llm.prepareCall(...).stream(request)` and
 * `llm.stream(request)` — both are wrapped so every request whose model is
 * text-only has its user-message image blocks described before the adapter
 * sees them. The originals are restored on dispose.
 *
 * @returns a disposer restoring the original methods.
 */
function installRequestTransform(ctx, config, settingsSource) {
	const cache = new Map();
	// `ctx.llm` is a traceable proxy; the raw service instance sits behind
	// the global cordis.original symbol, and the agent loop resolves the SAME
	// instance, so own-property method wraps here are visible to every caller.
	const raw = ctx.llm[Symbol.for("cordis.original")];
	const originalPrepareCall = raw.prepareCall;
	const originalStream = raw.stream;
	const transform = (request) => transformRequest(ctx, config, settingsSource, request, cache);
	raw.prepareCall = async function(config2, signal) {
		const prepared = await originalPrepareCall.call(raw, config2, signal);
		const preparedStream = prepared.stream;
		return {
			...prepared,
			stream: (options) => transformThenStream(options, preparedStream)
		};
	};
	raw.stream = function(options) {
		return transformStream(options);
	};
	async function* transformThenStream(options, preparedStream) {
		const transformed = await transform(options);
		yield* preparedStream(transformed);
	}
	async function* transformStream(options) {
		const transformed = await transform(options);
		yield* originalStream.call(raw, transformed);
	}
	return () => {
		raw.prepareCall = originalPrepareCall;
		raw.stream = originalStream;
	};
}
...
function apply(ctx, config) {
	...
	// The image-admission bridge: the patched host's `session.prompt` consults
	// this service when the session model is text-only and the user attached
	// images. The service disappears with this plugin, so `ctx.get` answering
	// it is exactly the "plugin is active" signal the host checks.
	ctx.provide("easyvision", {
		checkPromptContent: (content) => checkPromptContent(ctx, config, settingsSource, content)
	});
	// The request-time transform: every LLM dispatch whose model is text-only
	// has its user-message image blocks described through the vision model
	// before the adapter sees them, so the committed message keeps its real
	// images (the web chat shows the picture) while the model reads the
	// description. Restored when the plugin unloads.
	ctx.effect(() => installRequestTransform(ctx, config, settingsSource));
	ctx.inject(["attachments"], (imageCtx) => {
		applyDescribeImageTool(imageCtx, config, () => settingsSource());
	});
}

export { Config, EasyVisionBridgeError, apply, checkPromptContent, inject, installRequestTransform, name, transformRequest };
