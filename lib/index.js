...
/** Decode one wire image part with the host's canonical-base64 contract. */
function decodePromptImageData(data) {
	const decoded = Buffer.from(data, "base64");
	if (data.length === 0 || decoded.toString("base64") !== data) {
		throw new EasyVisionBridgeError("easyvision-image-invalid", "the attached image is not canonical base64; re-attach it and try again");
	}
 ...
 * attached images. The images are described through the configured vision
 * model and the prompt content is returned as text, so the message is
 * admitted with the description the main model can actually read. Any
 * configuration/validation failure throws {@link EasyVisionBridgeError} with
 * a wire code the host surfaces to the client.
 *
 * @param ctx - the harness context (llm/attachments services).
 ...
 * @returns text-only prompt parts: the original text plus the description.
 */
async function describePromptContent(ctx, config, settingsSource, content) {
	const resolved = effectiveConfig(config, settingsSource);
	await resolveVisionModel(ctx, resolved);
	const attachments = ctx.get("attachments");
	if (attachments === void 0) throw new EasyVisionBridgeError("easyvision-not-configured", "the durable attachment service is not mounted, so dsh-easyvision cannot process images");
	...
	if (imageParts.length > limits.maxImagesPerMessage) {
		throw new EasyVisionBridgeError("easyvision-image-invalid", `a message can include up to ${limits.maxImagesPerMessage} images`);
	}
	const prepared = imageParts.map((part) => ({ part, data: decodePromptImageData(part.data) }));
	const totalBytes = prepared.reduce((sum, item) => sum + item.data.byteLength, 0);
	if (totalBytes > limits.maxMessageImageBytes) {
		throw new EasyVisionBridgeError("easyvision-image-invalid", `the attached images exceed ${limits.maxMessageImageBytes} bytes in total; remove some and try again`);
	}
	for (const { part, data } of prepared) {
		try {
			...
			throw new EasyVisionBridgeError("easyvision-image-invalid", `an attached image failed EasyVision validation: ${error instanceof Error ? error.message : String(error)}`, { cause: error });
		}
	}
	const refs = [];
	for (const { part, data } of prepared) {
	...
	let description;
	try {
		({ description } = await describeWithVisionModel(ctx, resolved, message));
	} catch (error) {
		const cause = error instanceof Error ? error.message.replace(/^easyvision: /, "") : String(error);
		throw new EasyVisionBridgeError("easyvision-vision-failed", `the EasyVision vision call failed: ${cause}`, { cause: error });
	}
	const label = refs.length === 1 ? "Attached image" : `Attached images (${refs.length})`;
	return [
		...textParts,
		{ type: "text", text: `[${label} — described by ${resolved.provider}/${resolved.model} via EasyVision: ${description}]` }
	];
}
...
function apply(ctx, config) {
	...
	ctx.provide("easyvision", {
		describePromptContent: (content) => describePromptContent(ctx, config, settingsSource, content)
	});
...
export { Config, EasyVisionBridgeError, apply, describePromptContent, inject, name };
